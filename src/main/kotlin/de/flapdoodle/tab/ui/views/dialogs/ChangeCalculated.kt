...
class ChangeCalculated<K: Comparable<K>>(
...
) : AbstractDialogContent<Change.Calculation.Properties>() {
    ...
    override fun result(): Change.Calculation.Properties {
        ...
        return Change.Calculation.Properties(node.id, newName)
    }

    companion object {
        fun <K: Comparable<K>> open(node: Node.Calculated<K>): Change.Calculation.Properties? {
            val result = Dialogs.open { ChangeCalculated(node) }
