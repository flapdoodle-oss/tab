        ...
        if (_index.size > 1) {
            ...
            if (idx != -1) {
                return if (idx + 1 < _index.size) {
                    ...
                    Evaluated.ofNullable(valueType, interpolation.interpolate(start to values[start]!!, end to values[end]!!, offset))
                } else {
                    // after last index
                ...
                val end = _index[1]
                return Evaluated.ofNullable(valueType, interpolation.interpolate(start to values[start]!!, end to values[end]!!, offset))
            }
        }
        if (_index.size == 1) {
            return Evaluated.ofNullable(valueType, values[_index[0]])
        }
        return Evaluated.ofNull(valueType)
    }
}