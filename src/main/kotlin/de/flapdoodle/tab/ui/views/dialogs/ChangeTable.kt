...
class ChangeTable<K: Comparable<K>>(
...
) : AbstractDialogContent<Change.Table.Properties>() {
    ...
    override fun result(): Change.Table.Properties {
        ...
        return Change.Table.Properties(node.id, newName)
    }

    companion object {
        fun <K: Comparable<K>> open(node: Node.Table<K>): Change.Table.Properties? {
            val result = Dialogs.open { ChangeTable(node) }
