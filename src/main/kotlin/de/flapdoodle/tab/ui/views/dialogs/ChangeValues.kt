...
class ChangeValues(
...
) : AbstractDialogContent<Change.Constants.Properties>() {
    ...
    override fun result(): Change.Constants.Properties {
        val newName = Title(nameField.text, shortField.text, descriptionField.text)
        return Change.Constants.Properties(node.id, newName)
    }

    companion object {
        fun open(node: Node.Constants): Change.Constants.Properties? {
            val result = Dialogs.open { ChangeValues(node) }
            return if (result!=null && result.name != node.name) result else null
        }
    }
}